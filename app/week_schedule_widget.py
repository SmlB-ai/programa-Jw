...
class WeekScheduleWidget(QWidget):
    themeChanged = pyqtSignal(int, int, int, str) # week_type, row, col, new_text
    """
    Un widget para mostrar el horario de una sola semana con un diseño
    que imita la imagen de `Programa.png`.
    """
    def __init__(self, week_date, week_type, week_data, theme_overrides={}, parent=None):
    ...
    def build_normal_week_ui(self):
        try:
            ...
            display_theme_tesoros = self.theme_overrides.get((self.week_type, 3, 0), self.week_data[3][0])
            ...
            editable_theme_tesoros.textChanged.connect(lambda new_text: self.themeChanged.emit(self.week_type, 3, 0, new_text))
            ...
            # Vida
            display_vida_1_title = self.theme_overrides.get((self.week_type, 10, 1), self.week_data[10][1])
            vida_1_name = self.week_data[11][1]
            vida_1_layout = QVBoxLayout(); vida_1_layout.setSpacing(0)
            editable_vida_title = EditableLabel(display_vida_1_title, self)
            editable_vida_title.setFont(self._create_styled_label("", point_size=9, bold=True).font())
            editable_vida_title.textChanged.connect(lambda new_text: self.themeChanged.emit(self.week_type, 10, 1, new_text))
            vida_1_name_label = self._create_styled_label(vida_1_name, point_size=9)
            vida_1_layout.addWidget(editable_vida_title)
